...
def load_data(infile, proposal_id, input_catalog, epoch_str="J2000.0"):
    ...
    df["priority"] = [0] * df.index.size
    df["components"] = ['kdm'] * df.index.size
    ...
    df["psf_mag_y"] = np.full(df.index.size, 99.0, dtype=float)
    df["psf_mag_error_y"] = np.full(df.index.size, 99.0, dtype=float)
    df["psf_flux_y"] = df['f_y'] * 1e32
    df["psf_flux_error_y"] = df['e_y'] * 1e32
...
def main(conf, infile, dry_run=False, proposal_id=None, input_catalog=None):
    ...
    tag = infile.split('/')[-1].split('.cat')[0]
    df = fill_ob_code(df, tag)

    obj_id_inserted = []
    idx_remove = []
    for i, row in df.iterrows():
        if row['obj_id'] in obj_id_inserted:
            idx_remove.append(i)
        else:
            obj_id_inserted.append(row['obj_id'])
